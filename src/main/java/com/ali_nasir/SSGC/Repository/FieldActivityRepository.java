...
//        "f.schedDttm, f.foWorkedBy, f.foWorkedDttm, f.name, " +
//        "CONCAT(f.address1, ' ', f.city), " + // ✅ fullAddress
//        "f.meterNo, f.remarks, f.reason, f.bookRoute) " +
//        "FROM FieldActivity f " +
//        "WHERE (:unit IS NULL OR f.unit = :unit) " +
//        "AND (:region IS NULL OR f.region = :region) " +
//        "AND (:zone IS NULL OR f.zones = :zone) " +
//        "AND (:subzone IS NULL OR f.subzone = :subzone) " +
//        "AND (:area IS NULL OR f.areaDescr = :area) " +
//        "AND (:faType IS NULL OR f.faType = :faType) " +
//        "AND (:faStatus IS NULL OR f.faStatus = :faStatus) " +
//        "AND (:startDate IS NULL OR f.schedDttm >= :startDate) " +
//        "AND (:endDate IS NULL OR f.schedDttm <= :endDate)")
// List<FieldActivityDTO> searchActivities(
